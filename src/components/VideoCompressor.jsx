...
export default function VideoCompressor({ darkMode }) {
  ...
  const loadFFmpeg = async () => {
    if (!isLoaded) {
      try {
        ...
        setStatusMessage(`Failed to load FFmpeg: ${error.message} ❌`);
        console.error("FFmpeg load error:", error);
        throw error;
      }
    }
  ...
  const handleFileChange = (e) => {
    ...
    if (file && file.type.startsWith("video/")) {
      if (file.size > 100 * 1024 * 1024) {
        setStatusMessage("File too large (max 100MB)");
        return;
      }
      setVideoFile(file);
      setOutput(null);
  ...
  const compressVideo = async () => {
    ...
    try {
      ...
      let params = [];
      if (compressionMode === "fast") {
        params = ["-preset", "ultrafast", "-b:v", "1000k", "-vf", "scale=1280:720"];
      } else if (compressionMode === "balanced") {
        params = ["-preset", "medium", "-b:v", "2000k", "-vf", "scale=1280:720"];
      } else if (compressionMode === "high") {
        params = ["-preset", "slow", "-b:v", "3000k", "-vf", "scale=1280:720"];
      }
...
const styles = {
  ...
  title: {
    fontSize: window.innerWidth <= 600 ? "1.5rem" : "1.8rem",
    fontWeight: "700",
  },
  uploadBox: {
    border: "none",
